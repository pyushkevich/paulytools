...
public:
  ...
  void GetPointCoordinate(vtkIdType id, Vec &target)
    { target = CastVTK(m_Mesh->GetPoint(id)); }
  ...
  void GetPointCoordinateAndNormal(vtkIdType id, Vec &x, Vec &n)
    {
    x = CastVTK(m_Mesh->GetPoint(id));
    n = CastVTK(m_Mesh->GetPointData()->GetNormals()->GetTuple3(id));
    }
  ...
  void AddNewPoint(vtkIdType iPoint)
    ...
    Vec xControl = CastVTK(m_Mesh->GetPoint(iPoint));
  ...
  void AddMarker(vtkIdType idCell, const char *name, 
    ...
    for(unsigned int iPoint = 0; iPoint < nPoints; iPoint++)
      xCenter += CastVTK(m_Mesh->GetPoint(xPoints[iPoint]));
    xCenter /= 3.0;
  ...
  */
  ...
  void GetCellGeometry(vtkIdType idCell, 
    ...
    // Assign the corners
    x1 = CastVTK(m_Mesh->GetPoint(iPoints[0]));
    x2 = CastVTK(m_Mesh->GetPoint(iPoints[1]));
    x3 = CastVTK(m_Mesh->GetPoint(iPoints[2]));
