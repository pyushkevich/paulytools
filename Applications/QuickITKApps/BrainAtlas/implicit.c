...
typedef struct process {	   /* parameters, function, storage */
	...
	IMP_POINT start;		   /* start point on surface */
	CUBES *cubes;		   /* active cubes */
	IMP_VERTICES vertices;		   /* surface vertices */
...
IMP_TEST find (int sign, PROCESS *p, double x, double y, double z)
{
	int i;
	IMP_TEST test;
	double range = p->size;
...
void converge (IMP_POINT * p1, IMP_POINT * p2, double v, double (*function)(), IMP_POINT *p)
{
	int i = 0;
	IMP_POINT pos, neg;
	if (v < 0) {
...
void vnormal (IMP_POINT *point, PROCESS *p, IMP_POINT *v)
{
...
void addtovertices (IMP_VERTICES *vertices,IMP_VERTEX v)
{
	if (vertices->count == vertices->max) {
		int i;
		IMP_VERTEX *new;
		vertices->max = vertices->count == 0 ? 10 : 2*vertices->count;
		new = (IMP_VERTEX *) mycalloc(vertices->max, sizeof(IMP_VERTEX));
...
*		 IMP_VERTICES vertices (the vertex array, indexed from 0)
...
//	bgnclosedline();
//	for (i = 0; i < 3; i++) {
//		IMP_POINT *p = &vertices.ptr[ids[i]].position;
...
//IMP_VERTICES gvertices;  /* global needed by application */
//
//
