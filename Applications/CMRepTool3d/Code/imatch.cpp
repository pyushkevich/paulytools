...
void SplineBinaryVolumeMatcher::alignSplineByMoments() {

  // Compute the mean and covariance matrix of the non-zero voxels
  typedef vnl_matrix_fixed<float,3,3> Mat;
  typedef SMLVec3f Vec;

  Mat xCov(0.0f);
  Vec xMean(0.0f);
  int n = 0;

  // Go through all the voxels in the image
  int xc,yc,zc,x,y,z;  
  for(zc = 0; zc < bim->cube.size(2); zc++) 
    {
    for(yc = 0; yc < bim->cube.size(1); yc++) 
      {
      for(xc = 0; xc < bim->cube.size(0); xc++) 
        {
        DataCube<unsigned char> &ic = bim->cube(xc,yc,zc);
        for(z=0;z<ic.size(2)-1;z++) 
          {
          for(y=0;y<ic.size(1)-1;y++) 
            {
            for(x=0;x<ic.size(0)-1;x++) 
              {
              if(ic(x,y,z) != 0)
                {
                int px = (xc << bim->LDCUBE) + x;
                int py = (yc << bim->LDCUBE) + y;
                int pz = (zc << bim->LDCUBE) + z;

                // Find the coordinate of the point
                Vec xImage(1.0f * px, 1.0f * py, 1.0f * pz);
                Vec xSpace = TransformPoint(bim->IS,xImage);

                xMean += xSpace;
                xCov += outer_product(xSpace, xSpace);
                n++;
                }
              }
            }
  ...
  // Now go through the spline and compute its moments
  for(unsigned int i = 0; i < cache->patch.width(); i++)
    {
    for(unsigned int j = 0; j < cache->patch.height(); j++)
      {
  ...
  vnl_matrix<float> Vx(3,3), Vy(3,3);
  vnl_symmetric_eigensystem_compute(xCov,Vx,Dx);
  vnl_symmetric_eigensystem_compute(yCov,Vy,Dy);

  // Compute the rotation to apply to the spline

  // cout << "Rotate: " << R << endl;

  // Compute the scale factor
  float s = sqrt(dot_product(Dx,Dy) / dot_product(Dy,Dy));

  // Create a copy of the spline's control points
  Array2D<SMLVec4f> backup, best;
  spline->getControlArray(backup);
  spline->getControlArray(best);
  float bestMatch = 0;
  ...
  for(unsigned int f = 0; f<8; f++)
    {
    // Set the flip matrix
    vnl_matrix<float> flip(3,3,0.0f);
    flip(0,0) = (f & 1) ? -1.0f : 1.0f;
    flip(1,1) = (f & 2) ? -1.0f : 1.0f;
    flip(2,2) = (f & 4) ? -1.0f : 1.0f;

    // Compute the rotation+flip matrix
    vnl_matrix<float> R = Vx * flip * Vy.transpose();

    // Apply the rotation to all the control points
    
    for(u = 0; u <= spline->dim(0); u++)
      {
      for(v = 0; v <= spline->dim(1); v++)
        {
        Vec P(backup(u,v)(0),backup(u,v)(1),backup(u,v)(2));
        float r = backup(u,v)(3);
        P = s * (R * (P - yMean)) + xMean;
        spline->setControl(u,v,P);
        spline->setControl(u,v,3,r * s);
        }
      }

    // Compute the match with this flip factor
    float match = this->getMatch();
    if(match < bestMatch)
      {
      spline->getControlArray(best);
      bestMatch = match;
      cout << "Best match " << match << " for flip " << f << endl;
      }
    }

  // Apply the best control set
  for(u = 0; u <= spline->dim(0); u++)
    {
    for(v = 0; v <= spline->dim(1); v++)
      {
      spline->setControl(u,v,best(u,v));
      }
    }
}
