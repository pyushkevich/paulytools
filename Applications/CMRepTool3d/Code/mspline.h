...
public:
  ...
  // This method pushes the control points to move the spline
  void push(const SplineGridDefinition &grid,int iGrid,int jGrid,
    int iControlFirst,int iControlLast,float *vector) 
    {
    push(grid.knot(0,iGrid),grid.knot(1,jGrid),
      grid.uv(0,iGrid),grid.uv(1,jGrid),iControlFirst,iControlLast,vector);
    }
  ...
  // This method computes the basis function Nip at a specific value.  
  // Third parameter is an array of length 3, in it the 
...
public:
  ...
  MSpline(int m,int n, int nZeroKnots = 2);
  ...
  static bool interpolateBoundary(MedialPoint &MP);
  static void interpolateBoundaryCrest(MedialPoint &MP);
};
