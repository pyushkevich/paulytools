...
void BSpline1D::fitToPoints(const Matrix &Q) {
    int i;
    ...
    double l = 0;
    for(i=1;i<=m;i++) 
	l += (Q.getRow(i) - Q.getRow(i-1)).twoNorm();

    // Construct an array of u-values for the Q points
    vector<float> uk;
    uk.push_back(0.0f);
    for(i=1;i<m;i++) {
	uk.push_back(uk.back() + (Q.getRow(i) - Q.getRow(i-1)).twoNorm() / l);
    }
    uk.push_back(1.0f);
  ...
  int c1 = c0+3 < n ? c0+3 : n;
    ...
    Matrix R(n-1,Q.columns());
    Matrix N(m-1,n-1);

    // R is computed as a sum
    for(int c=1;c<=n-1;c++) {
	for(int r=1;r<=m-1;r++) {
	    Vector L = Q.getRow(r)-Q.getRow(0)*BN(r,0)-Q.getRow(m)*BN(r,n);
	    Vector Rk = R.getRow(c-1);
	    Rk += L * BN(r,c);
	    R.insertMatrix(c-1,0,Rk.t());
	}
    }
