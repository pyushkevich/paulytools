...
void EvolutionaryStrategy::computeRandomSigma(Vector &sigma) {
	...
	for(int i=0;i<ss.dimensions();i++) {
		sigma(i) = sigmaBase(i) * fastExp(getGaussianRnd2(0,1.0));
	}
}
...
void EvolutionaryStrategy::mutate(ESSolution *s1)
{
	...
	// Mutate sigma
	for(i=0;i<s1->x.size();i+=granularity) {
		if(pMutation == 1.0 || rand(1.0) < pMutation) { 
			for(int j=0;j<granularity;j++) {
				double newSigma;
				int count = 0;
				while(++count < 10) {
					// Compute the new sigma
					newSigma = s1->sigma(i+j) * fastExp(getGaussianRnd2(0,deltaSigma(i+j)),false);

					// Check that we are within bounds
					if(!sBounded || (newSigma >= sLowerBound(i+j) && newSigma <= sUpperBound(i+j))) 
						break;

					if(newSigma < sLowerBound(i+j))
						newSigma = sLowerBound(i+j);
					else
						newSigma = sUpperBound(i+j);
				}

				s1->sigma(i+j) = newSigma;

				double newX;
				count = 0;
				while(++count < 10) {
					// Compute the new x
					newX = getGaussianRnd2(s1->x(i+j),s1->sigma(i+j));

					// Check that we are within bounds
					if(!xBounded || (newX >= xLowerBound(i+j) && newX <= xUpperBound(i+j))) 
						break;

					if(newX < xLowerBound(i+j))
						newX = xLowerBound(i+j);
					else
						newX = xUpperBound(i+j);
				}
...
void EvolutionaryStrategy::selection()
{
	...
	for(i=mu;i<mu+lambda;i++) {
		if(nCrossoverParents==2) {
			...
			for(int j=0;j<nCrossoverParents;j++) {
				index[j] = rand(mu);
