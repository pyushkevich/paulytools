...
protected:
	...
	// Probability of a node's mutation
	double pMutation;
...
public:
	...
	/**
	* This is a smart way to set the sigmas to get a desired effect
	* Specify a vector B and a value Alpha, and all sigmas will be computed such
	* that
	* Product_i [ P(|X_i - Mean_i| < SSSigma_i) ] = Alpha
	*/
	...
	// Set probability of a value's mutation
	void setMutationProbability(double p) {
		pMutation = p;
	}

	// Set granularity: if set to 2 this means that elements 01 12 34... always mutate int the same iteration
